  ...
  async function startRedirectNavigation(state2, redirect8, _temp2) {
    ...
    if (isBrowser2) {
      ...
      if (redirect8.reloadDocument) {
        ...
        const url = init.history.createURL(redirect8.location);
...
var init_router = __esm({
  "node_modules/react-router/node_modules/@remix-run/router/dist/router.js"() {
    ...
    (function(ResultType5) {
      ResultType5["data"] = "data";
      ResultType5["deferred"] = "deferred";
      ResultType5["redirect"] = "redirect";
      ResultType5["error"] = "error";
...
__export(router_exports, {
  ...
  isDeferredData: () => isDeferredData2,
...
function createMemoryHistory2(options) {
  ...
  function createMemoryLocation(to, state, key) {
    ...
    warning2(location.pathname.charAt(0) === "/", "relative pathnames are not supported in memory history: " + JSON.stringify(to));
    return location;
  }
  function createHref(to) {
...
function convertRoutesToDataRoutes2(routes, mapRouteProperties2, parentPath, manifest) {
  if (parentPath === void 0) {
    parentPath = [];
  }
  if (manifest === void 0) {
...
function matchRoutes2(routes, locationArg, basename) {
  ...
  let matches = null;
  for (let i = 0; matches == null && i < branches.length; ++i) {
    let decoded = decodePath2(pathname);
    matches = matchRouteBranch2(branches[i], decoded);
  }
  return matches;
}
...
function createRouter2(init) {
  ...
  async function navigate(to, opts) {
    ...
    let normalizedPath = normalizeTo2(state.location, state.matches, basename, future.v7_prependBasename, to, future.v7_relativeSplatPath, opts == null ? void 0 : opts.fromRouteId, opts == null ? void 0 : opts.relative);
    let {
      path,
      submission,
      error
    } = normalizeNavigateOptions2(future.v7_normalizeFormMethod, false, normalizedPath, opts);
    let currentLocation = state.location;
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let loadingNavigation = opts && opts.overrideNavigation;
    let matches = matchRoutes2(routesToUse, location, basename);
    let flushSync = (opts && opts.flushSync) === true;
    if (!matches) {
      let error = getInternalRouterError2(404, {
        pathname: location.pathname
  ...
  async function handleAction(request, location, submission, matches, opts) {
    ...
    if (!actionMatch.route.action && !actionMatch.route.lazy) {
      ...
      result = await callLoaderOrAction2("action", request, actionMatch, matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath);
      if (request.signal.aborted) {
        return {
          shortCircuited: true
        };
  ...
  async function handleLoaders(request, location, matches, overrideNavigation, submission, fetcherSubmission, replace, initialHydration, flushSync, pendingActionData, pendingError) {
    ...
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad2(init.history, state, matches, activeSubmission, location, future.v7_partialHydration && initialHydration === true, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionData, pendingError);
    cancelActiveDeferreds((routeId) => !(matches && matches.some((m) => m.route.id === routeId)) || matchesToLoad && matchesToLoad.some((m) => m.route.id === routeId));
    pendingNavigationLoadId = ++incrementingLoadId;
    if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
      let updatedFetchers2 = markFetchRedirectsDone();
  ...
  function fetch2(key, routeId, href, opts) {
    ...
    let flushSync = (opts && opts.unstable_flushSync) === true;
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let normalizedPath = normalizeTo2(state.location, state.matches, basename, future.v7_prependBasename, href, future.v7_relativeSplatPath, routeId, opts == null ? void 0 : opts.relative);
    let matches = matchRoutes2(routesToUse, normalizedPath, basename);
    if (!matches) {
    ...
    let {
      ...
      submission,
      error
    } = normalizeNavigateOptions2(future.v7_normalizeFormMethod, true, normalizedPath, opts);
    if (error) {
      setFetcherError(key, routeId, error, {
        flushSync
      });
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, flushSync, submission) {
    ...
    fetchControllers.set(key, abortController);
    let originatingLoadId = incrementingLoadId;
    let actionResult = await callLoaderOrAction2("action", fetchRequest, match, requestMatches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath);
    if (fetchRequest.signal.aborted) {
      if (fetchControllers.get(key) === abortController) {
        fetchControllers.delete(key);
      }
    ...
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let matches = state.navigation.state !== "idle" ? matchRoutes2(routesToUse, state.navigation.location, basename) : state.matches;
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches, flushSync, submission) {
    ...
    fetchControllers.set(key, abortController);
    let originatingLoadId = incrementingLoadId;
    let result = await callLoaderOrAction2("loader", fetchRequest, match, matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath);
    if (isDeferredResult2(result)) {
  ...
  async function startRedirectNavigation(state2, redirect8, _temp2) {
    ...
    let activeSubmission = submission || fetcherSubmission;
    if (redirectPreserveMethodStatusCodes2.has(redirect8.status) && activeSubmission && isMutationMethod2(activeSubmission.formMethod)) {
      await startNavigation(redirectHistoryAction, redirectLocation, {
  ...
  async function callLoadersAndMaybeResolveData(currentMatches, matches, matchesToLoad, fetchersToLoad, request) {
    let results = await Promise.all([...matchesToLoad.map((match) => callLoaderOrAction2("loader", request, match, matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath)), ...fetchersToLoad.map((f) => {
      if (f.matches && f.match && f.controller) {
        return callLoaderOrAction2("loader", createClientSideRequest2(init.history, f.path, f.controller.signal), f.match, f.matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath);
      } else {
        let error = {
          type: ResultType2.error,
  ...
  function getScrollKey(location, matches) {
    if (getScrollRestorationKey) {
      let key = getScrollRestorationKey(location, matches.map((m) => convertRouteMatchToUiMatch2(m, state.loaderData)));
      return key || location.key;
    }
    return location.key;
  }
...
function createStaticHandler(routes, opts) {
  ...
  let dataRoutes = convertRoutesToDataRoutes2(routes, mapRouteProperties2, void 0, manifest);
  async function query(request, _temp3) {
    ...
    let matches = matchRoutes2(dataRoutes, location, basename);
  ...
  async function queryRoute(request, _temp4) {
    ...
    let match = routeId ? matches.find((m) => m.route.id === routeId) : getTargetMatch2(matches, location);
    if (routeId && !match) {
  ...
  async function queryImpl(request, location, matches, requestContext, routeMatch) {
    ...
    try {
      if (isMutationMethod2(request.method.toLowerCase())) {
        let result2 = await submit(request, matches, routeMatch || getTargetMatch2(matches, location), requestContext, routeMatch != null);
        return result2;
      }
  ...
  async function submit(request, matches, actionMatch, requestContext, isRouteRequest) {
    ...
    return _extends3({}, context, result.statusCode ? {
      statusCode: result.statusCode
    } : {}, {
      actionData: {
        [actionMatch.route.id]: result.data
  ...
  async function loadRouteData(request, matches, requestContext, routeMatch, pendingActionError) {
    ...
    let requestMatches = routeMatch ? [routeMatch] : getLoaderMatchesUntilBoundary2(matches, Object.keys(pendingActionError || {})[0]);
    ...
    let results = await Promise.all([...matchesToLoad.map((match) => callLoaderOrAction2("loader", request, match, matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath, {
      isStaticRequest: true,
      isRouteRequest,
      requestContext
    }))]);
    ...
    let activeDeferreds = /* @__PURE__ */ new Map();
    let context = processRouteLoaderData2(matches, matchesToLoad, results, pendingActionError, activeDeferreds);
...
function getMatchesToLoad2(history, state, matches, submission, location, isInitialLoad, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionData, pendingError) {
  let actionResult = pendingError ? Object.values(pendingError)[0] : pendingActionData ? Object.values(pendingActionData)[0] : void 0;
  let currentUrl = history.createURL(state.location);
  let nextUrl = history.createURL(location);
  let boundaryId = pendingError ? Object.keys(pendingError)[0] : void 0;
  ...
  let navigationMatches = boundaryMatches.filter((match, index) => {
    ...
    let currentRouteMatch = state.matches[index];
    let nextRouteMatch = match;
    return shouldRevalidateLoader2(match, _extends3({
      currentUrl,
      currentParams: currentRouteMatch.params,
      nextUrl,
      nextParams: nextRouteMatch.params
  ...
  fetchLoadMatches.forEach((f, key) => {
    ...
    if (fetchRedirectIds.has(key)) {
      ...
      shouldRevalidate = shouldRevalidateLoader2(fetcherMatch, _extends3({
        currentUrl,
        currentParams: state.matches[state.matches.length - 1].params,
        nextUrl,
        nextParams: matches[matches.length - 1].params
  ...
  try {
    ...
    if (match.route.lazy) {
      if (handler) {
        ...
        await loadLazyRouteModule2(match.route, mapRouteProperties2, manifest);
        handler = match.route[type];
        if (handler) {
          result = await runHandler(handler);
        } else if (type === "action") {
  ...
  if (isResponse2(result)) {
    ...
    if (redirectStatusCodes2.has(status)) {
      ...
      if (!ABSOLUTE_URL_REGEX2.test(location)) {
        location = normalizeTo2(new URL(request.url), matches.slice(0, matches.indexOf(match) + 1), basename, true, location, v7_relativeSplatPath);
      } else if (!opts.isStaticRequest) {
        let currentUrl = new URL(request.url);
        let url = location.startsWith("//") ? new URL(currentUrl.protocol + location) : new URL(location);
...
function processLoaderData2(state, matches, matchesToLoad, results, pendingError, revalidatingFetchers, fetcherResults, activeDeferreds) {
  ...
  for (let index = 0; index < revalidatingFetchers.length; index++) {
    ...
    if (controller && controller.signal.aborted) {
      ...
      let boundaryMatch = findNearestBoundary2(state.matches, match == null ? void 0 : match.route.id);
      if (!(errors && errors[boundaryMatch.route.id])) {
...
function getShortCircuitMatches2(routes) {
  let route = routes.length === 1 ? routes[0] : routes.find((r) => r.index || !r.path || r.path === "/") || {
    id: "__shim-error-route__"
  };
  return {
...
function isQueryRouteResponse(obj) {
  return obj && isResponse2(obj.response) && (obj.type === ResultType2.data || obj.type === ResultType2.error);
}
...
var Action2, PopStateEventType, ResultType2, immutableRouteKeys2, paramRe2, dynamicSegmentValue2, indexRouteValue2, emptySegmentValue2, staticSegmentValue2, splatPenalty2, isSplat2, joinPaths2, normalizePathname2, normalizeSearch2, normalizeHash2, json3, AbortedDeferredError2, DeferredData2, defer3, redirect3, redirectDocument2, ErrorResponseImpl2, validMutationMethodsArr2, validMutationMethods2, validRequestMethodsArr2, validRequestMethods2, redirectStatusCodes2, redirectPreserveMethodStatusCodes2, IDLE_NAVIGATION2, IDLE_FETCHER2, IDLE_BLOCKER2, ABSOLUTE_URL_REGEX2, defaultMapRouteProperties2, TRANSITIONS_STORAGE_KEY2, UNSAFE_DEFERRED_SYMBOL2;
var init_router2 = __esm({
  "node_modules/react-router-dom/node_modules/@remix-run/router/dist/router.js"() {
    ...
    paramRe2 = /^:[\w-]+$/;
    dynamicSegmentValue2 = 3;
    indexRouteValue2 = 2;
    emptySegmentValue2 = 1;
    staticSegmentValue2 = 10;
    splatPenalty2 = -2;
    isSplat2 = (s) => s === "*";
    joinPaths2 = (paths) => paths.join("/").replace(/\/\/+/g, "/");
    normalizePathname2 = (pathname) => pathname.replace(/\/+$/, "").replace(/^\/*/, "/");
    normalizeSearch2 = (search) => !search || search === "?" ? "" : search.startsWith("?") ? search : "?" + search;
    normalizeHash2 = (hash) => !hash || hash === "#" ? "" : hash.startsWith("#") ? hash : "#" + hash;
    json3 = function json4(data, init) {
      if (init === void 0) {
        init = {};
      }
      let responseInit = typeof init === "number" ? {
    ...
    redirect3 = function redirect4(url, init) {
      if (init === void 0) {
        init = 302;
      }
      let responseInit = init;
    ...
    redirectPreserveMethodStatusCodes2 = /* @__PURE__ */ new Set([307, 308]);
    IDLE_NAVIGATION2 = {
      state: "idle",
      location: void 0,
      formMethod: void 0,
    ...
    defaultMapRouteProperties2 = (route) => ({
      hasErrorBoundary: Boolean(route.hasErrorBoundary)
    });
...
function deserializeErrors(errors) {
  ...
  for (let [key, val] of entries) {
    if (val && val.__type === "RouteErrorResponse") {
      serialized[key] = new ErrorResponseImpl2(val.status, val.statusText, val.data, val.internal === true);
...
function RouterProvider2(_ref) {
  ...
  React2.useEffect(() => {
    true ? warning2(fallbackElement == null || !router2.future.v7_partialHydration, "`<RouterProvider fallbackElement>` is deprecated when using `v7_partialHydration`, use a `HydrateFallback` component instead") : void 0;
  }, []);
...
function useFetcher(_temp3) {
  ...
  let FetcherForm = React2.useMemo(() => {
    let FetcherForm2 = /* @__PURE__ */ React2.forwardRef((props, ref) => {
      return /* @__PURE__ */ React2.createElement(Form, _extends4({}, props, {
        navigate: false,
        fetcherKey,
        ref
      }));
  ...
  let fetcher = state.fetchers.get(fetcherKey) || IDLE_FETCHER2;
...
function useScrollRestoration(_temp4) {
  ...
  usePageHide(React2.useCallback(() => {
    ...
    try {
      ...
      true ? warning2(false, "Failed to save scroll positions in sessionStorage, <ScrollRestoration /> will not work properly (" + error + ").") : void 0;
    }
    window.history.scrollRestoration = "auto";
...
function useViewTransitionState(to, opts) {
  ...
  let currentPath = stripBasename2(vtContext.currentLocation.pathname, basename) || vtContext.currentLocation.pathname;
  let nextPath = stripBasename2(vtContext.nextLocation.pathname, basename) || vtContext.nextLocation.pathname;
  return matchPath2(path.pathname, nextPath) != null || matchPath2(path.pathname, currentPath) != null;
}
var React2, ReactDOM, defaultMethod, defaultEncType, _formDataSupportsSubmitter, supportedFormEncTypes, _excluded, _excluded2, _excluded3, REACT_ROUTER_VERSION, ViewTransitionContext, FetchersContext, START_TRANSITION2, startTransitionImpl2, FLUSH_SYNC, flushSyncImpl, USE_ID, useIdImpl, Deferred, isBrowser, ABSOLUTE_URL_REGEX3, Link, NavLink, Form, DataRouterHook2, DataRouterStateHook2, fetcherId, getUniqueFetcherId, SCROLL_RESTORATION_STORAGE_KEY, savedScrollPositions;
var init_dist2 = __esm({
  "node_modules/react-router-dom/dist/index.js"() {
    ...
    Link = /* @__PURE__ */ React2.forwardRef(function LinkWithRef(_ref7, ref) {
      ...
      if (typeof to === "string" && ABSOLUTE_URL_REGEX3.test(to)) {
        absoluteHref = to;
        if (isBrowser) {
          try {
    ...
    NavLink = /* @__PURE__ */ React2.forwardRef(function NavLinkWithRef(_ref8, ref) {
      ...
      if (nextLocationPathname && basename) {
        nextLocationPathname = stripBasename2(nextLocationPathname, basename) || nextLocationPathname;
      }
...
var require_server = __commonJS({
  "node_modules/react-router-dom/server.js"(exports) {
    ...
    var router2 = (init_router2(), __toCommonJS(router_exports));
    ...
    function StaticRouterProvider2({
      ...
      if (hydrate !== false) {
        ...
        let json8 = htmlEscape(JSON.stringify(JSON.stringify(data)));
        hydrateScript = `window.__staticRouterHydrationData = JSON.parse(${json8});`;
      }
      let {
        state
    ...
    function encodeLocation(to) {
      ...
      href = href.replace(/ $/, "%20");
      let encoded = ABSOLUTE_URL_REGEX6.test(href) ? new URL(href) : new URL(href, "http://localhost");
      return {
    ...
    var ABSOLUTE_URL_REGEX6 = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
    var ESCAPE_LOOKUP2 = {
      "&": "\\u0026",
      ">": "\\u003e",
      "<": "\\u003c",
...
var json5 = function json6(data, init) {
  if (init === void 0) {
    init = {};
  }
  let responseInit = typeof init === "number" ? {
...
var AbortedDeferredError3 = class extends Error {
};
var DeferredData3 = class {
  constructor(data, responseInit) {
    this.pendingKeysSet = /* @__PURE__ */ new Set();
    this.subscribers = /* @__PURE__ */ new Set();
    this.deferredKeys = [];
    invariant3(data && typeof data === "object" && !Array.isArray(data), "defer() only accepts plain objects");
    let reject;
    this.abortPromise = new Promise((_, r) => reject = r);
    this.controller = new AbortController();
  ...
  get unwrappedData() {
    invariant3(this.data !== null && this.done, "Can only unwrap data on initialized and settled deferreds");
    return Object.entries(this.data).reduce((acc, _ref3) => {
      let [key, value] = _ref3;
      return Object.assign(acc, {
...
var defer5 = function defer6(data, init) {
  if (init === void 0) {
    init = {};
  }
  let responseInit = typeof init === "number" ? {
...
var redirect5 = function redirect6(url, init) {
  if (init === void 0) {
    init = 302;
  }
  let responseInit = init;
...
var redirectDocument3 = (url, init) => {
  let response = redirect5(url, init);
  response.headers.set("X-Remix-Reload-Document", "true");
  return response;
};
var validMutationMethodsArr3 = ["post", "put", "patch", "delete"];
var validMutationMethods3 = new Set(validMutationMethodsArr3);
var validRequestMethodsArr3 = ["get", ...validMutationMethodsArr3];
var validRequestMethods3 = new Set(validRequestMethodsArr3);
var UNSAFE_DEFERRED_SYMBOL3 = Symbol("deferred");

// node_modules/@remix-run/server-runtime/dist/esm/responses.js
var json7 = (data, init = {}) => {
  return json5(data, init);
};
var defer7 = (data, init = {}) => {
  return defer5(data, init);
};
var redirect7 = (url, init = 302) => {
  return redirect5(url, init);
};
var redirectDocument4 = (url, init = 302) => {
  return redirectDocument3(url, init);
};
...
var immutableRouteKeys3 = /* @__PURE__ */ new Set(["lazy", "caseSensitive", "path", "id", "index", "children"]);
function isIndexRoute3(route) {
  return route.index === true;
}
function convertRoutesToDataRoutes3(routes, mapRouteProperties2, parentPath, manifest) {
  if (parentPath === void 0) {
    parentPath = [];
  }
  if (manifest === void 0) {
  ...
  return routes.map((route, index) => {
    ...
    if (isIndexRoute3(route)) {
      let indexRoute = _extends6({}, route, mapRouteProperties2(route), {
        id
      });
      manifest[id] = indexRoute;
      return indexRoute;
    } else {
      let pathOrLayoutRoute = _extends6({}, route, mapRouteProperties2(route), {
        id,
        children: void 0
      });
      manifest[id] = pathOrLayoutRoute;
      if (route.children) {
        pathOrLayoutRoute.children = convertRoutesToDataRoutes3(route.children, mapRouteProperties2, treePath, manifest);
      }
      return pathOrLayoutRoute;
    }
  });
...
function matchRoutes3(routes, locationArg, basename) {
  ...
  let pathname = stripBasename3(location.pathname || "/", basename);
  if (pathname == null) {
    return null;
  }
  let branches = flattenRoutes3(routes);
  rankRouteBranches3(branches);
  let matches = null;
  for (let i = 0; matches == null && i < branches.length; ++i) {
    let decoded = decodePath3(pathname);
    matches = matchRouteBranch3(branches[i], decoded);
  }
  return matches;
}
function convertRouteMatchToUiMatch3(match, loaderData) {
  let {
    route,
    pathname,
    params
...
function flattenRoutes3(routes, branches, parentsMeta, parentPath) {
  if (branches === void 0) {
    branches = [];
  }
  if (parentsMeta === void 0) {
  ...
  let flattenRoute = (route, index, relativePath) => {
    ...
    let path = joinPaths3([parentPath, meta.relativePath]);
    let routesMeta = parentsMeta.concat(meta);
    if (route.children && route.children.length > 0) {
      invariant4(
        // Our types know better, but runtime JS may not!
      ...
      flattenRoutes3(route.children, branches, routesMeta, path);
    }
    if (route.path == null && !route.index) {
      return;
    }
...
function explodeOptionalSegments3(path) {
  ...
  let restExploded = explodeOptionalSegments3(rest.join("/"));
  let result = [];
  result.push(...restExploded.map((subpath) => subpath === "" ? required : [required, subpath].join("/")));
  if (isOptional) {
    result.push(...restExploded);
...
function rankRouteBranches3(branches) {
  branches.sort((a, b) => a.score !== b.score ? b.score - a.score : compareIndexes3(a.routesMeta.map((meta) => meta.childrenIndex), b.routesMeta.map((meta) => meta.childrenIndex)));
}
var paramRe3 = /^:[\w-]+$/;
var dynamicSegmentValue3 = 3;
var indexRouteValue3 = 2;
var emptySegmentValue3 = 1;
var staticSegmentValue3 = 10;
var splatPenalty3 = -2;
var isSplat3 = (s) => s === "*";
function computeScore3(path, index) {
  let segments = path.split("/");
  let initialScore = segments.length;
  if (segments.some(isSplat3)) {
    initialScore += splatPenalty3;
  }
  if (index) {
    initialScore += indexRouteValue3;
  }
  return segments.filter((s) => !isSplat3(s)).reduce((score, segment) => score + (paramRe3.test(segment) ? dynamicSegmentValue3 : segment === "" ? emptySegmentValue3 : staticSegmentValue3), initialScore);
}
...
function matchRouteBranch3(branch, pathname) {
  ...
  for (let i = 0; i < routesMeta.length; ++i) {
    ...
    if (match.pathnameBase !== "/") {
      matchedPathname = joinPaths3([matchedPathname, match.pathnameBase]);
    }
  }
  return matches;
}
function matchPath3(pattern, pathname) {
  ...
  let [matcher, compiledParams] = compilePath3(pattern.path, pattern.caseSensitive, pattern.end);
  let match = pathname.match(matcher);
  if (!match)
    return null;
  let matchedPathname = match[0];
...
function resolvePath3(to, fromPathname) {
  ...
  let pathname = toPathname ? toPathname.startsWith("/") ? toPathname : resolvePathname3(toPathname, fromPathname) : fromPathname;
  return {
    pathname,
...
function getResolveToMatches3(matches, v7_relativeSplatPath) {
...
function resolveTo3(toArg, routePathnames, locationPathname, isPathRelative) {
  ...
  if (typeof toArg === "string") {
    ...
    to = _extends6({}, toArg);
    invariant4(!to.pathname || !to.pathname.includes("?"), getInvalidPathError3("?", "pathname", "search", to));
    invariant4(!to.pathname || !to.pathname.includes("#"), getInvalidPathError3("#", "pathname", "hash", to));
    invariant4(!to.search || !to.search.includes("#"), getInvalidPathError3("#", "search", "hash", to));
  }
  let isEmptyPath = toArg === "" || to.pathname === "";
  let toPathname = isEmptyPath ? "/" : to.pathname;
  let from;
...
var joinPaths3 = (paths) => paths.join("/").replace(/\/\/+/g, "/");
var normalizePathname3 = (pathname) => pathname.replace(/\/+$/, "").replace(/^\/*/, "/");
var normalizeSearch3 = (search) => !search || search === "?" ? "" : search.startsWith("?") ? search : "?" + search;
var normalizeHash3 = (hash) => !hash || hash === "#" ? "" : hash.startsWith("#") ? hash : "#" + hash;
var AbortedDeferredError4 = class extends Error {
};
var DeferredData4 = class {
  constructor(data, responseInit) {
    this.pendingKeysSet = /* @__PURE__ */ new Set();
    this.subscribers = /* @__PURE__ */ new Set();
    this.deferredKeys = [];
  ...
  onSettle(promise, key, error, data) {
    if (this.controller.signal.aborted && error instanceof AbortedDeferredError4) {
      this.unlistenAbortSignal();
      Object.defineProperty(promise, "_error", {
        get: () => error
      });
...
var validMutationMethodsArr4 = ["post", "put", "patch", "delete"];
var validMutationMethods4 = new Set(validMutationMethodsArr4);
var validRequestMethodsArr4 = ["get", ...validMutationMethodsArr4];
var validRequestMethods4 = new Set(validRequestMethodsArr4);
var redirectStatusCodes3 = /* @__PURE__ */ new Set([301, 302, 303, 307, 308]);
var redirectPreserveMethodStatusCodes3 = /* @__PURE__ */ new Set([307, 308]);
var IDLE_NAVIGATION3 = {
  state: "idle",
  location: void 0,
  formMethod: void 0,
  formAction: void 0,
...
var defaultMapRouteProperties3 = (route) => ({
  hasErrorBoundary: Boolean(route.hasErrorBoundary)
});
var TRANSITIONS_STORAGE_KEY3 = "remix-router-transitions";
function createRouter3(init) {
  const routerWindow = init.window ? init.window : typeof window !== "undefined" ? window : void 0;
  const isBrowser2 = typeof routerWindow !== "undefined" && typeof routerWindow.document !== "undefined" && typeof routerWindow.document.createElement !== "undefined";
  const isServer = !isBrowser2;
  ...
  let manifest = {};
  let dataRoutes = convertRoutesToDataRoutes3(init.routes, mapRouteProperties2, void 0, manifest);
  let inFlightDataRoutes;
  let basename = init.basename || "/";
  let future = _extends6({
    v7_fetcherPersist: false,
    v7_normalizeFormMethod: false,
    v7_partialHydration: false,
    v7_prependBasename: false,
  ...
  function initialize() {
    unlistenHistory = init.history.listen((_ref) => {
      ...
      if (blockerKey && delta != null) {
        ...
        updateBlocker(blockerKey, {
          ...
          reset() {
            let blockers = new Map(state.blockers);
            blockers.set(blockerKey, IDLE_BLOCKER3);
            updateState({
              blockers
            });
          }
    ...
    if (isBrowser2) {
      restoreAppliedTransitions3(routerWindow, appliedViewTransitions);
      let _saveAppliedTransitions = () => persistAppliedTransitions3(routerWindow, appliedViewTransitions);
      routerWindow.addEventListener("pagehide", _saveAppliedTransitions);
      removePageHideEventListener = () => routerWindow.removeEventListener("pagehide", _saveAppliedTransitions);
    }
    if (!state.initialized) {
      startNavigation(Action4.Pop, state.location, {
        initialHydration: true
      });
    }
    return router2;
  ...
  function updateState(newState, opts) {
    ...
    state = _extends6({}, state, newState);
    let completedFetchers = [];
    let deletedFetchersKeys = [];
    if (future.v7_fetcherPersist) {
      state.fetchers.forEach((fetcher, key) => {
  ...
  function completeNavigation(location, newState, _temp) {
    ...
    let isActionReload = state.actionData != null && state.navigation.formMethod != null && isMutationMethod3(state.navigation.formMethod) && state.navigation.state === "loading" && ((_location$state = location.state) == null ? void 0 : _location$state._isRedirect) !== true;
    let actionData;
    if (newState.actionData) {
      if (Object.keys(newState.actionData).length > 0) {
        actionData = newState.actionData;
    ...
    let loaderData = newState.loaderData ? mergeLoaderData3(state.loaderData, newState.loaderData, newState.matches || [], newState.errors) : state.loaderData;
    let blockers = state.blockers;
    if (blockers.size > 0) {
      blockers = new Map(blockers);
      blockers.forEach((_, k) => blockers.set(k, IDLE_BLOCKER3));
    }
    let preventScrollReset = pendingPreventScrollReset === true || state.navigation.formMethod != null && isMutationMethod3(state.navigation.formMethod) && ((_location$state2 = location.state) == null ? void 0 : _location$state2._isRedirect) !== true;
    if (inFlightDataRoutes) {
      dataRoutes = inFlightDataRoutes;
      inFlightDataRoutes = void 0;
    }
    if (isUninterruptedRevalidation)
      ;
    else if (pendingAction === Action4.Pop)
      ;
    else if (pendingAction === Action4.Push) {
      init.history.push(location, location.state);
    } else if (pendingAction === Action4.Replace) {
      init.history.replace(location, location.state);
    }
    let viewTransitionOpts;
    if (pendingAction === Action4.Pop) {
      let priorPaths = appliedViewTransitions.get(state.location.pathname);
      if (priorPaths && priorPaths.has(location.pathname)) {
        viewTransitionOpts = {
          currentLocation: state.location,
    ...
    updateState(_extends6({}, newState, {
      actionData,
      loaderData,
      historyAction: pendingAction,
      location,
  ...
  async function navigate(to, opts) {
    ...
    let normalizedPath = normalizeTo3(state.location, state.matches, basename, future.v7_prependBasename, to, future.v7_relativeSplatPath, opts == null ? void 0 : opts.fromRouteId, opts == null ? void 0 : opts.relative);
    let {
      path,
      submission,
      error
    } = normalizeNavigateOptions3(future.v7_normalizeFormMethod, false, normalizedPath, opts);
    let currentLocation = state.location;
    let nextLocation = createLocation3(state.location, path, opts && opts.state);
    nextLocation = _extends6({}, nextLocation, init.history.encodeLocation(nextLocation));
    let userReplace = opts && opts.replace != null ? opts.replace : void 0;
    let historyAction = Action4.Push;
    if (userReplace === true) {
      historyAction = Action4.Replace;
    } else if (userReplace === false)
      ;
    else if (submission != null && isMutationMethod3(submission.formMethod) && submission.formAction === state.location.pathname + state.location.search) {
      historyAction = Action4.Replace;
    }
    let preventScrollReset = opts && "preventScrollReset" in opts ? opts.preventScrollReset === true : void 0;
    let flushSync = (opts && opts.unstable_flushSync) === true;
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    if (state.initialized && !isRevalidationRequired && isHashChangeOnly3(state.location, location) && !(opts && opts.submission && isMutationMethod3(opts.submission.formMethod))) {
      completeNavigation(location, {
        matches
      }, {
        flushSync
    ...
    pendingNavigationController = new AbortController();
    let request = createClientSideRequest3(init.history, location, pendingNavigationController.signal, opts && opts.submission);
    let pendingActionData;
    let pendingError;
    if (opts && opts.pendingError) {
      pendingError = {
      ...
      pendingActionData = actionOutput.pendingActionData;
      pendingError = actionOutput.pendingActionError;
      loadingNavigation = getLoadingNavigation3(location, opts.submission);
      flushSync = false;
      request = new Request(request.url, {
        signal: request.signal
      });
    ...
    pendingNavigationController = null;
    completeNavigation(location, _extends6({
      matches
    }, pendingActionData ? {
      actionData: pendingActionData
    } : {}, {
  ...
  async function handleAction(request, location, submission, matches, opts) {
    ...
    interruptActiveLoads();
    let navigation = getSubmittingNavigation3(location, submission);
    updateState({
      navigation
    }, {
      flushSync: opts.flushSync === true
    ...
    let result;
    let actionMatch = getTargetMatch3(matches, location);
    if (!actionMatch.route.action && !actionMatch.route.lazy) {
      result = {
        type: ResultType4.error,
      ...
      result = await callLoaderOrAction3("action", request, actionMatch, matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath);
      if (request.signal.aborted) {
        return {
          shortCircuited: true
        };
  ...
  async function handleLoaders(request, location, matches, overrideNavigation, submission, fetcherSubmission, replace, initialHydration, flushSync, pendingActionData, pendingError) {
    let loadingNavigation = overrideNavigation || getLoadingNavigation3(location, submission);
    let activeSubmission = submission || fetcherSubmission || getSubmissionFromNavigation3(loadingNavigation);
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad3(init.history, state, matches, activeSubmission, location, future.v7_partialHydration && initialHydration === true, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionData, pendingError);
    cancelActiveDeferreds((routeId) => !(matches && matches.some((m) => m.route.id === routeId)) || matchesToLoad && matchesToLoad.some((m) => m.route.id === routeId));
    pendingNavigationLoadId = ++incrementingLoadId;
    if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
      let updatedFetchers2 = markFetchRedirectsDone();
    ...
    if (!isUninterruptedRevalidation && (!future.v7_partialHydration || !initialHydration)) {
      revalidatingFetchers.forEach((rf) => {
        let fetcher = state.fetchers.get(rf.key);
        let revalidatingFetcher = getLoadingFetcher3(void 0, fetcher ? fetcher.data : void 0);
        state.fetchers.set(rf.key, revalidatingFetcher);
      });
      let actionData = pendingActionData || state.actionData;
      updateState(_extends6({
        navigation: loadingNavigation
      }, actionData ? Object.keys(actionData).length === 0 ? {
        actionData: null
      } : {
    ...
    revalidatingFetchers.forEach((rf) => fetchControllers.delete(rf.key));
    let redirect8 = findRedirect3(results);
    if (redirect8) {
      if (redirect8.idx >= matchesToLoad.length) {
        let fetcherKey = revalidatingFetchers[redirect8.idx - matchesToLoad.length].key;
        fetchRedirectIds.add(fetcherKey);
    ...
    let {
      loaderData,
      errors
    } = processLoaderData3(state, matches, matchesToLoad, loaderResults, pendingError, revalidatingFetchers, fetcherResults, activeDeferreds);
    activeDeferreds.forEach((deferredData, routeId) => {
      deferredData.subscribe((aborted) => {
        if (aborted || deferredData.done) {
          activeDeferreds.delete(routeId);
  ...
  function fetch2(key, routeId, href, opts) {
    ...
    let flushSync = (opts && opts.unstable_flushSync) === true;
    let routesToUse = inFlightDataRoutes || dataRoutes;
    let normalizedPath = normalizeTo3(state.location, state.matches, basename, future.v7_prependBasename, href, future.v7_relativeSplatPath, routeId, opts == null ? void 0 : opts.relative);
    ...
    let {
      ...
      submission,
      error
    } = normalizeNavigateOptions3(future.v7_normalizeFormMethod, true, normalizedPath, opts);
    if (error) {
      setFetcherError(key, routeId, error, {
        flushSync
      });
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, flushSync, submission) {
    ...
    let existingFetcher = state.fetchers.get(key);
    updateFetcherState(key, getSubmittingFetcher3(submission, existingFetcher), {
      flushSync
    });
    let abortController = new AbortController();
    let fetchRequest = createClientSideRequest3(init.history, path, abortController.signal, submission);
    fetchControllers.set(key, abortController);
    let originatingLoadId = incrementingLoadId;
    let actionResult = await callLoaderOrAction3("action", fetchRequest, match, requestMatches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath);
    if (fetchRequest.signal.aborted) {
      if (fetchControllers.get(key) === abortController) {
        fetchControllers.delete(key);
      }
    ...
    if (future.v7_fetcherPersist && deletedFetchers.has(key)) {
      if (isRedirectResult3(actionResult) || isErrorResult3(actionResult)) {
        updateFetcherState(key, getDoneFetcher3(void 0));
        return;
      }
    } else {
      if (isRedirectResult3(actionResult)) {
        ...
        if (pendingNavigationLoadId > originatingLoadId) {
          ...
          fetchRedirectIds.add(key);
          updateFetcherState(key, getLoadingFetcher3(submission));
          return startRedirectNavigation(state, actionResult, {
            fetcherSubmission: submission
          });
        }
    ...
    let nextLocation = state.navigation.location || state.location;
    let revalidationRequest = createClientSideRequest3(init.history, nextLocation, abortController.signal);
    let routesToUse = inFlightDataRoutes || dataRoutes;
    ...
    let loadId = ++incrementingLoadId;
    fetchReloadIds.set(key, loadId);
    let loadFetcher = getLoadingFetcher3(submission, actionResult.data);
    state.fetchers.set(key, loadFetcher);
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad3(
      init.history,
      state,
      matches,
      submission,
    ...
    revalidatingFetchers.filter((rf) => rf.key !== key).forEach((rf) => {
      let staleKey = rf.key;
      let existingFetcher2 = state.fetchers.get(staleKey);
      let revalidatingFetcher = getLoadingFetcher3(void 0, existingFetcher2 ? existingFetcher2.data : void 0);
      state.fetchers.set(staleKey, revalidatingFetcher);
      if (fetchControllers.has(staleKey)) {
        abortFetcher(staleKey);
      }
    ...
    let {
      loaderData,
      errors
    } = processLoaderData3(state, state.matches, matchesToLoad, loaderResults, void 0, revalidatingFetchers, fetcherResults, activeDeferreds);
    if (state.fetchers.has(key)) {
      let doneFetcher = getDoneFetcher3(actionResult.data);
      state.fetchers.set(key, doneFetcher);
    }
    abortStaleFetchLoads(loadId);
    if (state.navigation.state === "loading" && loadId > pendingNavigationLoadId) {
      ...
      updateState({
        errors,
        loaderData: mergeLoaderData3(state.loaderData, loaderData, matches, errors),
        fetchers: new Map(state.fetchers)
      });
      isRevalidationRequired = false;
    }
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches, flushSync, submission) {
    let existingFetcher = state.fetchers.get(key);
    updateFetcherState(key, getLoadingFetcher3(submission, existingFetcher ? existingFetcher.data : void 0), {
      flushSync
    });
    let abortController = new AbortController();
    let fetchRequest = createClientSideRequest3(init.history, path, abortController.signal);
    fetchControllers.set(key, abortController);
    let originatingLoadId = incrementingLoadId;
    let result = await callLoaderOrAction3("loader", fetchRequest, match, matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath);
    if (isDeferredResult3(result)) {
      result = await resolveDeferredData3(result, fetchRequest.signal, true) || result;
    }
    if (fetchControllers.get(key) === abortController) {
      fetchControllers.delete(key);
    }
    ...
    updateFetcherState(key, getDoneFetcher3(result.data));
  }
  async function startRedirectNavigation(state2, redirect8, _temp2) {
    let {
      submission,
    ...
    if (isBrowser2) {
      ...
      if (redirect8.reloadDocument) {
        isDocumentReload = true;
      } else if (ABSOLUTE_URL_REGEX4.test(redirect8.location)) {
        const url = init.history.createURL(redirect8.location);
        isDocumentReload = // Hard reload if it's an absolute URL to a new origin
        url.origin !== routerWindow.location.origin || // Hard reload if it's an absolute URL that does not match our basename
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace === true ? Action4.Replace : Action4.Push;
    let {
      formMethod,
      formAction,
      formEncType
    ...
    if (!submission && !fetcherSubmission && formMethod && formAction && formEncType) {
      submission = getSubmissionFromNavigation3(state2.navigation);
    }
    let activeSubmission = submission || fetcherSubmission;
    if (redirectPreserveMethodStatusCodes3.has(redirect8.status) && activeSubmission && isMutationMethod3(activeSubmission.formMethod)) {
      await startNavigation(redirectHistoryAction, redirectLocation, {
        submission: _extends6({}, activeSubmission, {
          formAction: redirect8.location
        }),
        // Preserve this flag across redirects
        preventScrollReset: pendingPreventScrollReset
      ...
      let overrideNavigation = getLoadingNavigation3(redirectLocation, submission);
      await startNavigation(redirectHistoryAction, redirectLocation, {
        overrideNavigation,
        // Send fetcher submissions through for shouldRevalidate
        fetcherSubmission,
  ...
  async function callLoadersAndMaybeResolveData(currentMatches, matches, matchesToLoad, fetchersToLoad, request) {
    let results = await Promise.all([...matchesToLoad.map((match) => callLoaderOrAction3("loader", request, match, matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath)), ...fetchersToLoad.map((f) => {
      if (f.matches && f.match && f.controller) {
        return callLoaderOrAction3("loader", createClientSideRequest3(init.history, f.path, f.controller.signal), f.match, f.matches, manifest, mapRouteProperties2, basename, future.v7_relativeSplatPath);
      } else {
        let error = {
          type: ResultType4.error,
    ...
    let loaderResults = results.slice(0, matchesToLoad.length);
    let fetcherResults = results.slice(matchesToLoad.length);
    await Promise.all([resolveDeferredResults3(currentMatches, matchesToLoad, loaderResults, loaderResults.map(() => request.signal), false, state.loaderData), resolveDeferredResults3(currentMatches, fetchersToLoad.map((f) => f.match), fetcherResults, fetchersToLoad.map((f) => f.controller ? f.controller.signal : null), true)]);
    return {
      results,
      loaderResults,
      fetcherResults
  ...
  function getFetcher(key) {
    ...
    return state.fetchers.get(key) || IDLE_FETCHER3;
  }
  function deleteFetcher(key) {
    let fetcher = state.fetchers.get(key);
    if (fetchControllers.has(key) && !(fetcher && fetcher.state === "loading" && fetchReloadIds.has(key))) {
  ...
  function markFetchersDone(keys) {
    for (let key of keys) {
      let fetcher = getFetcher(key);
      let doneFetcher = getDoneFetcher3(fetcher.data);
      state.fetchers.set(key, doneFetcher);
    }
  }
  function markFetchRedirectsDone() {
  ...
  function getBlocker(key, fn) {
    let blocker = state.blockers.get(key) || IDLE_BLOCKER3;
    if (blockerFunctions.get(key) !== fn) {
      blockerFunctions.set(key, fn);
    }
    return blocker;
  ...
  function enableScrollRestoration(positions, getPosition, getKey) {
    ...
    getScrollPosition = getPosition;
    getScrollRestorationKey = getKey || null;
    if (!initialScrollRestored && state.navigation === IDLE_NAVIGATION3) {
      initialScrollRestored = true;
      let y = getSavedScrollPosition(state.location, state.matches);
      if (y != null) {
        updateState({
  ...
  function getScrollKey(location, matches) {
    if (getScrollRestorationKey) {
      let key = getScrollRestorationKey(location, matches.map((m) => convertRouteMatchToUiMatch3(m, state.loaderData)));
      return key || location.key;
    }
    return location.key;
  }
  ...
  function _internalSetRoutes(newRoutes) {
    manifest = {};
    inFlightDataRoutes = convertRoutesToDataRoutes3(newRoutes, mapRouteProperties2, void 0, manifest);
  }
  router2 = {
    get basename() {
      return basename;
...
var UNSAFE_DEFERRED_SYMBOL4 = Symbol("deferred");
function isSubmissionNavigation3(opts) {
  return opts != null && ("formData" in opts && opts.formData != null || "body" in opts && opts.body !== void 0);
}
function normalizeTo3(location, matches, basename, prependBasename, to, v7_relativeSplatPath, fromRouteId, relative) {
  let contextualMatches;
  let activeRouteMatch;
  if (fromRouteId) {
    contextualMatches = [];
  ...
  let path = resolveTo3(to ? to : ".", getResolveToMatches3(contextualMatches, v7_relativeSplatPath), stripBasename3(location.pathname, basename) || location.pathname, relative === "path");
  if (to == null) {
    path.search = location.search;
    path.hash = location.hash;
  }
  if ((to == null || to === "" || to === ".") && activeRouteMatch && activeRouteMatch.route.index && !hasNakedIndexQuery3(path.search)) {
    path.search = path.search ? path.search.replace(/^\?/, "?index&") : "?index";
  }
  if (prependBasename && basename !== "/") {
    path.pathname = path.pathname === "/" ? basename : joinPaths3([basename, path.pathname]);
  }
  return createPath3(path);
}
function normalizeNavigateOptions3(normalizeFormMethod, isFetcher, path, opts) {
  if (!opts || !isSubmissionNavigation3(opts)) {
    return {
      path
    };
  }
  ...
  let rawFormMethod = opts.formMethod || "get";
  let formMethod = normalizeFormMethod ? rawFormMethod.toUpperCase() : rawFormMethod.toLowerCase();
  let formAction = stripHashFromPath3(path);
  if (opts.body !== void 0) {
    if (opts.formEncType === "text/plain") {
  ...
  let formData;
  if (opts.formData) {
    searchParams = convertFormDataToSearchParams3(opts.formData);
    formData = opts.formData;
  } else if (opts.body instanceof FormData) {
    searchParams = convertFormDataToSearchParams3(opts.body);
    formData = opts.body;
  } else if (opts.body instanceof URLSearchParams) {
    searchParams = opts.body;
    formData = convertSearchParamsToFormData3(searchParams);
  } else if (opts.body == null) {
    searchParams = new URLSearchParams();
    formData = new FormData();
  } else {
  ...
  if (isFetcher && parsedPath.search && hasNakedIndexQuery3(parsedPath.search)) {
    searchParams.append("index", "");
  }
  parsedPath.search = "?" + searchParams;
  return {
...
function getMatchesToLoad3(history, state, matches, submission, location, isInitialLoad, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionData, pendingError) {
  let actionResult = pendingError ? Object.values(pendingError)[0] : pendingActionData ? Object.values(pendingActionData)[0] : void 0;
  let currentUrl = history.createURL(state.location);
  let nextUrl = history.createURL(location);
  let boundaryId = pendingError ? Object.keys(pendingError)[0] : void 0;
  let boundaryMatches = getLoaderMatchesUntilBoundary3(matches, boundaryId);
  let navigationMatches = boundaryMatches.filter((match, index) => {
    let {
      route
    } = match;
    ...
    if (isNewLoader3(state.loaderData, state.matches[index], match) || cancelledDeferredRoutes.some((id) => id === match.route.id)) {
      return true;
    }
    let currentRouteMatch = state.matches[index];
    let nextRouteMatch = match;
    return shouldRevalidateLoader3(match, _extends6({
      currentUrl,
      currentParams: currentRouteMatch.params,
      nextUrl,
      nextParams: nextRouteMatch.params
      ...
      defaultShouldRevalidate: (
        ...
        isRevalidationRequired || // Clicked the same link, resubmitted a GET form
        currentUrl.pathname + currentUrl.search === nextUrl.pathname + nextUrl.search || // Search params affect all loaders
        currentUrl.search !== nextUrl.search || isNewRouteInstance3(currentRouteMatch, nextRouteMatch)
      )
    }));
  });
  let revalidatingFetchers = [];
  fetchLoadMatches.forEach((f, key) => {
    ...
    let fetcher = state.fetchers.get(key);
    let fetcherMatch = getTargetMatch3(fetcherMatches, f.path);
    let shouldRevalidate = false;
    if (fetchRedirectIds.has(key)) {
      shouldRevalidate = false;
    } else if (cancelledFetcherLoads.includes(key)) {
      ...
      shouldRevalidate = shouldRevalidateLoader3(fetcherMatch, _extends6({
        currentUrl,
        currentParams: state.matches[state.matches.length - 1].params,
        nextUrl,
        nextParams: matches[matches.length - 1].params
...
function isNewLoader3(currentLoaderData, currentMatch, match) {
  let isNew = (
    // [a] -> [a, b]
    !currentMatch || // [a, b] -> [a, c]
    match.route.id !== currentMatch.route.id
...
function isNewRouteInstance3(currentMatch, match) {
  let currentPath = currentMatch.route.path;
  return (
    // param change for this match, /users/123 -> /users/456
    currentMatch.pathname !== match.pathname || // splat param changed, which is not present in match.path
...
function shouldRevalidateLoader3(loaderMatch, arg) {
  if (loaderMatch.route.shouldRevalidate) {
    let routeChoice = loaderMatch.route.shouldRevalidate(arg);
    if (typeof routeChoice === "boolean") {
      return routeChoice;
...
async function loadLazyRouteModule3(route, mapRouteProperties2, manifest) {
  ...
  for (let lazyRouteProperty in lazyRoute) {
    ...
    if (!isPropertyStaticallyDefined && !immutableRouteKeys3.has(lazyRouteProperty)) {
      routeUpdates[lazyRouteProperty] = lazyRoute[lazyRouteProperty];
    }
  }
  Object.assign(routeToUpdate, routeUpdates);
  Object.assign(routeToUpdate, _extends6({}, mapRouteProperties2(routeToUpdate), {
    lazy: void 0
  }));
}
async function callLoaderOrAction3(type, request, match, matches, manifest, mapRouteProperties2, basename, v7_relativeSplatPath, opts) {
  if (opts === void 0) {
    opts = {};
  }
  let resultType;
  ...
  try {
    ...
    if (match.route.lazy) {
      if (handler) {
        ...
        await loadLazyRouteModule3(match.route, mapRouteProperties2, manifest);
        handler = match.route[type];
        if (handler) {
          result = await runHandler(handler);
        } else if (type === "action") {
  ...
  if (isResponse3(result)) {
    ...
    if (redirectStatusCodes3.has(status)) {
      ...
      if (!ABSOLUTE_URL_REGEX4.test(location)) {
        location = normalizeTo3(new URL(request.url), matches.slice(0, matches.indexOf(match) + 1), basename, true, location, v7_relativeSplatPath);
      } else if (!opts.isStaticRequest) {
        let currentUrl = new URL(request.url);
        let url = location.startsWith("//") ? new URL(currentUrl.protocol + location) : new URL(location);
        let isSameBasename = stripBasename3(url.pathname, basename) != null;
        if (url.origin === currentUrl.origin && isSameBasename) {
          location = url.pathname + url.search + url.hash;
        }
      }
    ...
    if (opts.isRouteRequest) {
      let queryRouteResponse = {
        type: resultType === ResultType4.error ? ResultType4.error : ResultType4.data,
        response: result
      };
      throw queryRouteResponse;
    }
    ...
    if (resultType === ResultType4.error) {
      return {
        type: resultType,
...
function createClientSideRequest3(history, location, signal, submission) {
  let url = history.createURL(stripHashFromPath3(location)).toString();
  let init = {
    signal
  };
  if (submission && isMutationMethod3(submission.formMethod)) {
    ...
    if (formEncType === "application/json") {
      ...
      init.body = convertFormDataToSearchParams3(submission.formData);
    } else {
      init.body = submission.formData;
    }
  }
...
function convertFormDataToSearchParams3(formData) {
  let searchParams = new URLSearchParams();
  for (let [key, value] of formData.entries()) {
    searchParams.append(key, typeof value === "string" ? value : value.name);
  }
...
function convertSearchParamsToFormData3(searchParams) {
  let formData = new FormData();
  for (let [key, value] of searchParams.entries()) {
    formData.append(key, value);
  }
...
function processRouteLoaderData3(matches, matchesToLoad, results, pendingError, activeDeferreds) {
  ...
  results.forEach((result, index) => {
    let id = matchesToLoad[index].route.id;
    invariant4(!isRedirectResult3(result), "Cannot handle redirect results in processLoaderData");
    if (isErrorResult3(result)) {
      let boundaryMatch = findNearestBoundary3(matches, id);
      let error = result.error;
      if (pendingError) {
        error = Object.values(pendingError)[0];
        pendingError = void 0;
...
function processLoaderData3(state, matches, matchesToLoad, results, pendingError, revalidatingFetchers, fetcherResults, activeDeferreds) {
  let {
    loaderData,
    errors
  ...
  for (let index = 0; index < revalidatingFetchers.length; index++) {
    ...
    if (controller && controller.signal.aborted) {
      continue;
    } else if (isErrorResult3(result)) {
      let boundaryMatch = findNearestBoundary3(state.matches, match == null ? void 0 : match.route.id);
      if (!(errors && errors[boundaryMatch.route.id])) {
        errors = _extends6({}, errors, {
          [boundaryMatch.route.id]: result.error
        });
      }
      state.fetchers.delete(key);
      ...
      let doneFetcher = getDoneFetcher3(result.data);
      state.fetchers.set(key, doneFetcher);
    }
  }
  return {
...
function mergeLoaderData3(loaderData, newLoaderData, matches, errors) {
  let mergedLoaderData = _extends6({}, newLoaderData);
  for (let match of matches) {
    let id = match.route.id;
    if (newLoaderData.hasOwnProperty(id)) {
      if (newLoaderData[id] !== void 0) {
...
function getShortCircuitMatches3(routes) {
  let route = routes.length === 1 ? routes[0] : routes.find((r) => r.index || !r.path || r.path === "/") || {
    id: "__shim-error-route__"
  };
  return {
...
function getInternalRouterError3(status, _temp5) {
  ...
  return new ErrorResponseImpl3(status || 500, statusText, new Error(errorMessage), true);
}
function findRedirect3(results) {
  for (let i = results.length - 1; i >= 0; i--) {
    let result = results[i];
    if (isRedirectResult3(result)) {
      return {
...
function stripHashFromPath3(path) {
  let parsedPath = typeof path === "string" ? parsePath3(path) : path;
  return createPath3(_extends6({}, parsedPath, {
    hash: ""
  }));
}
function isHashChangeOnly3(a, b) {
  if (a.pathname !== b.pathname || a.search !== b.search) {
    return false;
  }
  if (a.hash === "") {
...
function isDeferredResult3(result) {
  return result.type === ResultType4.deferred;
}
function isErrorResult3(result) {
  return result.type === ResultType4.error;
}
function isRedirectResult3(result) {
  return (result && result.type) === ResultType4.redirect;
}
...
function isValidMethod3(method) {
  return validRequestMethods4.has(method.toLowerCase());
}
function isMutationMethod3(method) {
  return validMutationMethods4.has(method.toLowerCase());
}
async function resolveDeferredResults3(currentMatches, matchesToLoad, results, signals, isFetcher, currentLoaderData) {
  for (let index = 0; index < results.length; index++) {
    let result = results[index];
    let match = matchesToLoad[index];
    if (!match) {
    ...
    let currentMatch = currentMatches.find((m) => m.route.id === match.route.id);
    let isRevalidatingLoader = currentMatch != null && !isNewRouteInstance3(currentMatch, match) && (currentLoaderData && currentLoaderData[match.route.id]) !== void 0;
    if (isDeferredResult3(result) && (isFetcher || isRevalidatingLoader)) {
      ...
      await resolveDeferredData3(result, signal, isFetcher).then((result2) => {
        if (result2) {
          results[index] = result2 || results[index];
        }
      });
...
async function resolveDeferredData3(result, signal, unwrap) {
  if (unwrap === void 0) {
    unwrap = false;
  }
  let aborted = await result.deferredData.resolveData(signal);
...
function getSubmissionFromNavigation3(navigation) {
  let {
    formMethod,
    formAction,
    formEncType,
...
function getLoadingNavigation3(location, submission) {
  if (submission) {
    let navigation = {
      state: "loading",
      location,
...
function getSubmittingNavigation3(location, submission) {
  let navigation = {
    state: "submitting",
    location,
    formMethod: submission.formMethod,
...
function getLoadingFetcher3(submission, data) {
  if (submission) {
    let fetcher = {
      state: "loading",
      formMethod: submission.formMethod,
...
function getSubmittingFetcher3(submission, existingFetcher) {
  let fetcher = {
    state: "submitting",
    formMethod: submission.formMethod,
    formAction: submission.formAction,
...
function restoreAppliedTransitions3(_window, transitions) {
  try {
    let sessionPositions = _window.sessionStorage.getItem(TRANSITIONS_STORAGE_KEY3);
    if (sessionPositions) {
      let json8 = JSON.parse(sessionPositions);
      for (let [k, v] of Object.entries(json8 || {})) {
        if (v && Array.isArray(v)) {
...
function persistAppliedTransitions3(_window, transitions) {
  if (transitions.size > 0) {
    let json8 = {};
    for (let [k, v] of transitions) {
      json8[k] = [...v];
    ...
    try {
      _window.sessionStorage.setItem(TRANSITIONS_STORAGE_KEY3, JSON.stringify(json8));
    } catch (error) {
...
function useDataRouterContext3() {
  ...
  invariant5(context, "You must render this element inside a <DataRouterContext.Provider> element");
  return context;
}
function useDataRouterStateContext() {
  let context = React3.useContext(DataRouterStateContext);
  invariant5(context, "You must render this element inside a <DataRouterStateContext.Provider> element");
  return context;
}
...
function useRemixContext() {
  ...
  invariant5(context, "You must render this element inside a <Remix> element");
  return context;
}
...
var ABSOLUTE_URL_REGEX5 = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
var NavLink2 = /* @__PURE__ */ React3.forwardRef(({
  to,
  prefetch = "none",
  ...props
}, forwardedRef) => {
  let isAbsolute = typeof to === "string" && ABSOLUTE_URL_REGEX5.test(to);
  let href = useHref(to);
  let [shouldPrefetch, ref, prefetchHandlers] = usePrefetchBehavior(prefetch, props);
  return /* @__PURE__ */ React3.createElement(React3.Fragment, null, /* @__PURE__ */ React3.createElement(NavLink, _extends7({}, props, prefetchHandlers, {
...
var Link2 = /* @__PURE__ */ React3.forwardRef(({
  ...
  let href = useHref(to);
  let [shouldPrefetch, ref, prefetchHandlers] = usePrefetchBehavior(prefetch, props);
  return /* @__PURE__ */ React3.createElement(React3.Fragment, null, /* @__PURE__ */ React3.createElement(Link, _extends7({}, props, prefetchHandlers, {
    ref: mergeRefs(forwardedRef, ref),
    to
  })), shouldPrefetch && !isAbsolute ? /* @__PURE__ */ React3.createElement(PrefetchPageLinks, {
    page: href
...
function Links() {
  ...
  return /* @__PURE__ */ React3.createElement(React3.Fragment, null, criticalCss ? /* @__PURE__ */ React3.createElement("style", {
    ...
    key,
    link
  }) => isPageLinkDescriptor(link) ? /* @__PURE__ */ React3.createElement(PrefetchPageLinks, _extends7({
    key
  }, link)) : /* @__PURE__ */ React3.createElement("link", _extends7({
    key
  }, link))));
}
function PrefetchPageLinks({
  ...
  return /* @__PURE__ */ React3.createElement(PrefetchPageLinksImpl, _extends7({
    page,
    matches
  }, dataLinkProps));
}
...
function PrefetchPageLinksImpl({
  ...
  return /* @__PURE__ */ React3.createElement(React3.Fragment, null, dataHrefs.map((href) => /* @__PURE__ */ React3.createElement("link", _extends7({
    key: href,
    rel: "prefetch",
    as: "fetch",
    href
  }, linkProps))), moduleHrefs.map((href) => /* @__PURE__ */ React3.createElement("link", _extends7({
    key: href,
    rel: "modulepreload",
    href
    ...
    /* @__PURE__ */ React3.createElement("link", _extends7({
      key
    }, link))
  )));
}
function Meta() {
  ...
  return /* @__PURE__ */ React3.createElement(React3.Fragment, null, meta.flat().map((metaProps) => {
    ...
    if ("tagName" in metaProps) {
      ...
      let Comp = tagName;
      return /* @__PURE__ */ React3.createElement(Comp, _extends7({
    ...
    return /* @__PURE__ */ React3.createElement("meta", _extends7({
...
import(${JSON.stringify(manifest.entry.module)});`;
    return /* @__PURE__ */ React3.createElement(React3.Fragment, null, /* @__PURE__ */ React3.createElement("script", _extends7({}, props, {
      suppressHydrationWarning: true,
      dangerouslySetInnerHTML: createHtml(contextScript),
      type: void 0
    })), /* @__PURE__ */ React3.createElement("script", _extends7({}, props, {
      suppressHydrationWarning: true,
  ...
  let nextMatches = React3.useMemo(() => {
    if (navigation.location) {
      ...
      invariant5(matches2, `No routes match path "${navigation.location.pathname}"`);
      return matches2;
    }
    return [];
...
function DeferredHydrationScript({
  ...
  if (typeof document === "undefined" && deferredData && dataKey && routeId) {
    invariant5(deferredData.pendingKeys.includes(dataKey), `Deferred data for route ${routeId} with key ${dataKey} was not pending but tried to render a script for it.`);
  }
  return /* @__PURE__ */ React3.createElement(React3.Suspense, {
    fallback: (
      ...
      typeof document === "undefined" && deferredData && dataKey && routeId ? null : /* @__PURE__ */ React3.createElement("script", _extends7({}, scriptProps, {
        async: true,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: {
          __html: " "
    ...
    children: (data) => {
      return /* @__PURE__ */ React3.createElement("script", _extends7({}, scriptProps, {
        async: true,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: {
  ...
  }) : /* @__PURE__ */ React3.createElement("script", _extends7({}, scriptProps, {
    async: true,
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
      __html: " "
...
function deserializeErrors2(errors) {
  ...
  for (let [key, val] of entries) {
    if (val && val.__type === "RouteErrorResponse") {
      serialized[key] = new ErrorResponseImpl3(val.status, val.statusText, val.data, val.internal === true);
    } else if (val && val.__type === "Error") {
      if (val.__subType) {
        let ErrorConstructor = window[val.__subType];
        if (typeof ErrorConstructor === "function") {
...
function isNetworkErrorResponse(response) {
  return isResponse4(response) && response.status >= 400 && response.headers.get("X-Remix-Error") == null && response.headers.get("X-Remix-Catch") == null && response.headers.get("X-Remix-Response") == null;
}
    ...
    void (async () => {
      try {
        ...
        for (let [key, resolver] of Object.entries(deferredResolvers)) {
          resolver.reject(new AbortedDeferredError4(`Deferred ${key} will never be resolved`));
        }
      } catch (error) {
...
function createServerRoutes(manifest, routeModules, future, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), spaModeLazyPromise = Promise.resolve({
  ...
  return (routesByParentId[parentId] || []).map((route) => {
    ...
    invariant5(routeModule, "No `routeModule` available to create server routes");
    let dataRoute = {
...
function preventInvalidServerHandlerCall(type, route, isSpaMode) {
  if (isSpaMode) {
    ...
    console.error(msg2);
    throw new ErrorResponseImpl3(400, "Bad Request", new Error(msg2), true);
  }
  let fn = type === "action" ? "serverAction()" : "serverLoader()";
  let msg = `You are trying to call ${fn} on a route that does not have a server ${type} (routeId: "${route.id}")`;
  if (type === "loader" && !route.hasLoader || type === "action" && !route.hasAction) {
    console.error(msg);
    throw new ErrorResponseImpl3(400, "Bad Request", new Error(msg), true);
  }
}
function noActionDefinedError(type, routeId) {
  ...
  console.error(msg);
  throw new ErrorResponseImpl3(405, "Method Not Allowed", new Error(msg), true);
}
    ...
    if (routeModule) {
      ...
      dataRoute.loader = async ({
        ...
        try {
          let result = await prefetchStylesAndCallHandler(async () => {
            invariant5(routeModule, "No `routeModule` available for critical-route loader");
      ...
      dataRoute.action = ({
        ...
        return prefetchStylesAndCallHandler(async () => {
          invariant5(routeModule, "No `routeModule` available for critical-route action");
...
function ScrollRestoration2({
  ...
  return /* @__PURE__ */ React8.createElement("script", _extends7({}, props, {
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
...
export {
  ...
  json7 as json,
  defer7 as defer,
  redirect7 as redirect,
  redirectDocument4 as redirectDocument,
  RemixContext,
  NavLink2 as NavLink,
  Link2 as Link,
  Links,
...
@remix-run/react/dist/esm/index.js:
   ...
   *)
*/
//# sourceMappingURL=/build/_shared/chunk-G74YHI74.js.map
